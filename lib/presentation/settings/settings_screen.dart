    ...
    switch (index) {
      case 0:
        return _AppearanceSettings(
            key: const ValueKey(0), accentColor: accentColor);
      case 1:
        return _PlayerSettings(
            key: const ValueKey(1), accentColor: accentColor);
      case 2:
        return _LibrarySettings(
            key: const ValueKey(2), accentColor: accentColor);
      case 3:
        return _AdvancedSettings(
            key: const ValueKey(3), accentColor: accentColor);
      default:
...
class _AppearanceSettings extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(children: [
      const _SectionHeader(title: "Theme & Colors"),
      _SettingsCard(
        child: Padding(
      ...
      _SettingsCard(
        child: SwitchListTile(
          ...
          subtitle: const Text(
              "Show the anime you are watching in your Discord status",
      ...
      children: [
        ...
        _SettingsCard(
          ...
          children: [
            SwitchListTile(
              activeThumbColor: accentColor,
              title: const Text("Auto-skip Openings/Endings",
                  style: TextStyle(color: Colors.white)),
              subtitle: const Text(
                  "Automatically skip if AniSkip finds timestamps",
                  style: TextStyle(color: Colors.white38)),
        ...
        const SizedBox(height: 24),
        const _SectionHeader(title: "Languages (Priority)"),
        _SettingsCard(
          child: Column(
            children: [
              ListTile(
                ...
                trailing: DropdownButton<String>(
                  ...
                  value:
                      settings.audioLanguage.startsWith('jpn') ? 'jpn' : 'eng',
                  items: [
                    const DropdownMenuItem(
                        value: 'jpn',
                        child: Text("Japanese (Original)",
                            style: TextStyle(color: Colors.white))),
                    const DropdownMenuItem(
                        value: 'eng',
                  ...
                  onChanged: (val) {
                    if (val == 'jpn') {
                      notifier.updateAudioLang('jpn,jp,eng,en,rus,ru');
                    }
                    if (val == 'eng') {
                      notifier.updateAudioLang('eng,en,jpn,jp,rus,ru');
                    }
                  },
                ),
              ),
        ...
        const SizedBox(height: 24),
        const _SectionHeader(title: "Video Decoder"),
        _SettingsCard(
          child: ListTile(
            title: const Text("Hardware Acceleration",
                style: TextStyle(color: Colors.white)),
            subtitle: const Text(
                "Change this if video is lagging or shows a black screen",
                style: TextStyle(color: Colors.white38)),
            trailing: DropdownButton<String>(
              dropdownColor: const Color(0xFF1A1A1A),
              ...
              items: const [
                ...
                DropdownMenuItem(
                    value: 'd3d11va',
                    child: Text("Windows (d3d11va)",
                        style: TextStyle(color: Colors.white))),
                DropdownMenuItem(
                    value: 'no',
                    child: Text("Disabled (CPU)",
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
            Icon(icon,
                color: isSelected ? accentColor : Colors.white54, size: 22),
...
class _LibrarySettings extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(
      children: [
        const _SectionHeader(title: "Scanned Folders"),
        _SettingsCard(
          child: Column(
                ...
                ...folders.map(
                  (folder) => ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () => ref
                          .read(savedFoldersProvider.notifier)
                          .removeFolder(folder),
                  ...
                  if (folderPath != null) {
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text("Folder added to scan queue"),
      ...
      children: [
        ...
        _SettingsCard(
          child: ListTile(
            ...
            subtitle: const Text(
                "Removes all titles, watch history, and notes. Media files on disk will NOT be deleted.",
                style: TextStyle(color: Colors.white38)),
            onTap: () {
              showDialog(
                context: context,
