...
class LeftControlPanel extends ConsumerWidget {
  ...
  void _playEpisode(BuildContext context, String path) {
    if (!File(path).existsSync()) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("Ошибка:  Файл видео не найден!"),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final nextEpisodePath = anime.nextEpisodeToPlay ??
        (sortedEpisodes.isNotEmpty ? sortedEpisodes.first : '');
    final nextEpisodeIndex = sortedEpisodes.isNotEmpty
        ? sortedEpisodes.indexOf(nextEpisodePath) + 1
        : 0;
      ...
      children: [
                ...
                Transform.translate(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(
                        fontSize: 16, fontWeight: FontWeight.bold, height: 1.1),
                  ),
                ),
        ...
        Row(
          children: [
            Expanded(
                flex: 2,
                child: _StatusMenu(anime: anime, accentColor: accentColor)),
            const SizedBox(width: 8),
            Expanded(
                flex: 1,
                child: _HoverButton(
                  ...
                  onTap: () => AnimeDialogs.showCollectionDialog(
                      context, ref, anime, accentColor),
                )),
            const SizedBox(width: 8),
            Expanded(
                flex: 1,
      ...
      menuChildren: [
        MenuItemButton(
          onPressed: () => ref
              ...
              .updateStatus(anime.id, AnimeStatus.planned),
          child: const Padding(
        ...
        MenuItemButton(
          onPressed: () => ref
              .read(animeRepoProvider)
              .updateStatus(anime.id, AnimeStatus.watching),
          child: const Padding(
              padding: EdgeInsets.symmetric(vertical: 8, horizontal: 16),
              child:
                  Text("Смотрю", style: TextStyle(color: Colors.blueAccent))),
        ),
        MenuItemButton(
          onPressed: () => ref
              .read(animeRepoProvider)
              .updateStatus(anime.id, AnimeStatus.completed),
          child: const Padding(
              padding: EdgeInsets.symmetric(vertical: 8, horizontal: 16),
              child:
                  Text("Просмотрено", style: TextStyle(color: Colors.green))),
        ),
      ],
          ...
          child: InkWell(
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  Text(
                    anime.status == AnimeStatus.planned
                        ? "В планах"
                    ...
                    style: TextStyle(
                      ...
                      color: anime.status == AnimeStatus.planned
                          ? Colors.white
                          : anime.status == AnimeStatus.watching
                              ? Colors.blueAccent
                              : anime.status == AnimeStatus.completed
    ...
    return Theme(
      ...
      child: PopupMenuButton<String>(
        ...
        itemBuilder: (context) => [
          ...
          const PopupMenuItem(
              ...
              child: Row(children: [
                ...
                SizedBox(width: 12),
                Text("Отметить всё просмотренным",
        ...
        onSelected: (value) async {
          ...
          if (value == 'edit') {
            AnimeDialogs.showEditDialog(context, ref, anime, accentColor);
          }
          if (value == 'stats') {
            AnimeDialogs.showAnimeStatsDialog(context, anime, accentColor);
          }
          if (value == 'mark_all') {
            for (var path in anime.sortedEpisodes) {
              if (!anime.watchedEpisodes.contains(path)) {
                await ref
                    .read(animeRepoProvider)
                    .markEpisodeAsWatched(anime.id, path);
              }
            }
          }
        },
        child: _HoverButton(
            icon: Icons.more_horiz, tooltip: "Опции", accentColor: accentColor),
      ),
    );
  }
}
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                  color: isHovered ? widget.accentColor : Colors.white12),
            ),
            child: Icon(widget.icon,
                color: isHovered ? Colors.white : Colors.white70, size: 24),
          ),
        ),
      ),
    );
