...
class FileDiscoveryService {
  ...
  static final _extensionRegex =
      RegExp(r'\.(mkv|mp4|avi)$', caseSensitive: false);
  ...
  Future<void> scanAndGroupFiles(String folderPath) async {
    try {
      final groupedAnime =
          await Isolate.run(() => _scanDirectoryIsolated(folderPath))
              .timeout(const Duration(minutes: 2));

      if (groupedAnime.isEmpty) return;

      final animesToSave = <Anime>[];
      final shikimori = ShikimoriService();

      for (var entry in groupedAnime.entries) {
        final title = entry.key;
        final episodes = entry.value;
        final anime = await _repository.getAnimeByTitle(title);
  ...
  static String _parseAnimeTitle(String fileName) {
    var title = fileName.replaceAll(_extensionRegex, '');
    title = title.replaceAll(
        RegExp(r'\d{3,4}p|x264|x265|h264|h265|hevc|hi10p',
            caseSensitive: false),
        '');
    ...
    title = title.replaceAll(
        RegExp(r'\s+(?:ep|episode|серия)?\s?\d+(?:\s?v\d+)?$',
            caseSensitive: false),
        '');
